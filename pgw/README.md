...
- `GET /config/channels`, `GET /config/outputs` — текущая конфигурация (пароли скрыты)
- `GET /config/outputs/{name}/map` — Register Map в CSV
...
- **OPC UA**: security policy выбирается через `CoreClientUtils.SelectEndpoint(url, useSecurity)`
  ...
  предложенных сервером. Browse (`POST /config/opcua/browse`) не реализован.
  Аутентификация по сертификату переиспользует собственный сертификат шлюза, отдельного flow нет.
...
- Redundancy (§14.1), CSV import, Web-UI — вне v0.1 согласно самому ТЗ (§12, этап 7+).
